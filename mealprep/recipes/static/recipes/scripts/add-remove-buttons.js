...
*/
$(document).ready(function () { // runs when the page is read
    // list of elemens that get a click handler
  $("[data-id='add_to_active'], [data-id='remove_from_active'], [data-id='add_to_favorites'], [data-id='remove_from_favorites']").click(function (event) {
    // on click, extract this information from the html element
    var page_type = $(this).attr("data-page-type");
    var recipe_id = $(this).attr("data-recipe-id");
    var operation_type = $(this).attr("data-id");
    // make a post request to the favorite_active_update view with the paramaters from the html element
    $.post({
        ...
        // if post request is successful, update the UI without reloading
        ...
        return false; // prevents a reload
  });
});
